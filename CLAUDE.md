...
## Start here
- Read `loop/STATUS.md` first — auto-generated (~60 lines): current state, open release items with line refs, latest reports/handoffs, recent commits.
- Follow its line refs to read only the sections you need (`sed -n '167,190p' loop/RELEASE_ROADMAP.md`).
- Never load `loop/RELEASE_ROADMAP.md` (62KB, human-facing) wholesale.
- `loop/STATUS.md` is generated by `loop/gen_status.sh` — never hand-edit it.
