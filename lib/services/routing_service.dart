...
class RoutingService {
  ...
  /// Valhalla 라우팅 그래프가 "가능"으로 잘못 판단했던 좌회전 2건
  /// (마스터가 직접 주행하며 실측 확인 — loop/RECON_impossible_left_turns.md).
  /// 두 지점 다 중앙분리대로 물리 분리된 편도 간선도로인데, 그래프가 좌회전
  /// 가능으로 단순 처리한 것이 원인이었다. 근본 수정은
  /// docker/impossible_turn_overlays/(오버레이 패치 + 격리 검증 파이프라인)로
  /// 2026-07-21 프로덕션 타일에 이미 반영·검증됨 — 이 목록은 이제 "고치기
  /// 전까지의 임시 조치"가 아니라, 향후 pbf 갱신 때 오버레이 재적용을 깜박해도
  /// 조용히 회귀를 막아주는 2차 방어선이다. `avoid_locations`는 해당 지점
  /// 근방에 페널티만 주고(하드 차단 아님) 모든 라우팅 요청에 항상 포함시킨다.
  ...
  static Future<List<RouteResult>> fetchRoutes({
    ...
    // 코스별 costing_options — 앱 시작 시 서버에서 로드, 실패 시 내장 기본값 사용.
    // 파라미터 상세: RoutingConfig._defaultCostingOptions 참조.
    final costingOptions = RoutingConfig.costingOptions;
