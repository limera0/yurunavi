...
#### S15 상세 — 이어서 안내하기 + 투어 히스토리 병합  `상태: [x]` — **코드 완료(4청크 전부 PASS), 실기기 검증 대기**

> **2026-08-10 착수 순서 예외**: 확정 원칙상 P0~P2 잔여 5건(S1b·S9·S11·S13·S14) 완료 후
> P3 착수였으나, S11(고급휘발유 미표시)만 아직 `[ ]`인 채로 마스터가 오늘 S15 진행을
> 명시 지시 — 우선순위 예외로 S15 먼저 진행, S11은 별도로 남겨둠(완료된 것 아님).
>
> **구현 완료(2026-08-10 저녁, 전부 code-auditor PASS, `flutter analyze` 0 ·
> `flutter test` 714/714)**: 설계 전문은
> [HANDOFF_0810_S15_resume_navigation.md](HANDOFF_0810_S15_resume_navigation.md).
>
> | 커밋 | 청크 | 내용 |
> |---|---|---|
> | `2d7839e` | 1 | 목적지 스냅샷 저장(`ActiveTourDestinationStore`, `tour_<id>.dest.json`) + `TourLog.resumedFromId` + `TourRecoveryService.findResumableOrphan`/`finalizeAsInterrupted` |
> | `0450f89` | 2 | 재개 임계치 설정 — `resumeThresholdHoursProvider`(기본 2시간, 1~24시간 선택), 설정화면 셀렉터 |
> | `6b44c60` | 3 | 재개 트리거·프롬프트·재탐색 — 스플래시에서 재개 가능 판정 → "이어서 안내할까요?" 확인 → 수락 시 현재위치→원목적지 재탐색 후 `NavScreen` 진입, 거절/실패 시 데이터 유실 없이 정상 폴백 |
> | `33d37d9` | 4 | 히스토리 화면 병합 표시 — `resumedFromId` 체인을 id로 연결(`groupResumedTourLogs`), "이어서 안내됨" 배지 + 합산 거리/시간 카드 |
>
> 청크3+4는 위임했던 서브에이전트 2건이 API 월 사용량 한도로 중간에 죽어(정책상
> 재시도 대신 즉시 보고) 컨트롤 세션이 직접 마무리했다 — 청크3 1차 감사에서
> `mounted` 체크 누락 1건 FAIL, 수정 후 재감사 PASS.
>
> **남은 건 실기기/가상GPS 검증뿐**(코드 작업 없음): 주행 중 강제 종료 → 임계치
> 이내 재실행 → 프롬프트 확인 → 재개 시 재탐색 정상 진입 → 히스토리 병합 카드
> 확인. 임계치 초과 케이스(설정 낮추고 대기)도 별도 확인.

- [x] **재개 트리거: 앱 (재)시작 시 자동 제안** (마스터 확정, 수동재개 아님) — 스플래시에서
      "이어서 안내할까요?" 다이얼로그로 제시(커밋 `6b44c60`)
- [x] **오탐 방지 임계치 확정(2026-08-10)** — 설정 화면에 사용자가 시간 단위로 선택
      가능한 옵션 추가. 기본값 **2시간**, 최대 **24시간**. 그 이상 지난 고아 트랙은
      재개 제안하지 않고 기존처럼 히스토리 복구만 수행.
- [x] **재개 방식 확정(2026-08-10)**: 원경로 유지 아님 — **현재위치 → 원목적지 재탐색**으로
      확정(제안대로 승인). 이유: 중단 중 위치 이동 가능성, 기존 `routing_service` 재사용으로
      단순화(memory `feedback_prefer_simple_reuse` 방침)
- [x] `TourRecoveryService`(`tour_recovery_service.dart`) 확장 — 재개 판정 로직(임계치+
      목적지 사이드카) 추가, `findResumableOrphan`/`finalizeAsInterrupted` 신설(커밋 `2d7839e`)
- [x] 히스토리 **병합** — 중단 전/재개 후 두 구간을 히스토리 목록에 하나의 투어로 표시
      (표시 전용 병합, 원본 레코드는 그대로 유지 — 커밋 `33d37d9`)
