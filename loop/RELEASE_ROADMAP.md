...
- **🚨 2026-07-15 최신, 최우선**: POI 데이터소스 아키텍처 결함 발견 — 서비스키가 클라이언트에
  박혀있어 전 사용자가 쿼터 하나를 공유하는 구조라 실사용자 증가 시 서비스 자체가 불가능함이
  확인됨(오늘 실제로 개발자 1인 테스트만으로 일일 쿼터 소진, HTTP 429 라이브 확인됨). 15번
  항목으로 등록, 상세 설계·근거·진행순서는 `loop/HANDOFF_0715_poi_quota.md`를 먼저 읽고 바로
  착수할 것 — 이 문서보다 그 핸드오프가 최신 소스.
...
| 15 | POI 데이터소스 자체 호스팅 전환 (쿼터 아키텍처 결함 해소) | 특급 | TODO — 2026-07-15 발견, 상세는 `loop/HANDOFF_0715_poi_quota.md` |
...
### 15. POI 데이터소스 자체 호스팅 전환 — TODO (특급, 출시 자체를 막는 스케일링 결함)

- **발견 경위 (2026-07-15)**: `RIDE_RESULTS_0715.md`의 P0(POI 미표시) 수정을 재빌드·재설치
  했는데도 사용자가 재현 보고 → 서비스키로 실 API를 직접 curl 호출해보니 `HTTP 429 "API
  token quota exceeded"` — 빌드 문제가 아니라 개발계정 일일 쿼터(10,000건/일) 소진.
- **왜 특급인가**: 서비스키가 클라이언트(APK)에 그대로 박혀 있어 **전 사용자가 쿼터 하나를
  공유**하는 구조. 개발자 1인의 테스트+코드감사 중 반복 curl만으로도 소진됐는데, 실사용자가
  수백~수천 명이 되면 각자의 GPS 이동에 따른 폴링(현재 앱에 독립 폴링 경로 3곳 — 홈/내비
  ambient 레이어, 검색 프리페치)이 무조건 하루 안에 쿼터를 소진시킴. 운영계정 전환으로
  쿼터를 늘려도 "사용자 요청마다 외부 API를 직접 때리는" 아키텍처 자체는 안 바뀌어 사용자
  수가 더 늘면 다시 막힘 — 즉 배포 후 사용자가 늘수록 필연적으로 터지는 결함.
- **근본 해결책**: 이 API는 실시간 조회와 별개로 같은 데이터를 **분기별 전국 단위 CSV로도
  무료 배포**함([파일데이터](https://www.data.go.kr/data/15083033/fileData.do), 로그인
  불필요, "이용허락범위 제한없음", 다음 갱신 2026-08-01). 이 CSV를 `native/`(navi 백엔드,
  이미 axum+docker로 운영 중)에 분기 1회 동기화해 로컬 DB(SQLite+R-tree 제안)에 적재하고,
  신규 `/poi/nearby` 엔드포인트로 서빙 → 클라이언트는 data.go.kr 대신
  `navi.westinx.com`을 호출(`routing_service.dart`/`native_engine.dart`와 동일 패턴). 외부
  API 호출이 분기 1회 동기화뿐이라 사용자 수와 무관하게 쿼터 문제 원천 해소, 부수적으로
  서비스키가 APK에서 완전히 제거됨(보안 개선).
- **상세 설계·기존 코드 조사(native/main.rs·api.rs·Cargo.toml 구조, 진행 순서 8단계)는
  `loop/HANDOFF_0715_poi_quota.md` 참조** — 새 세션은 이 핸드오프부터 읽고 착수할 것.
- 착수 전 남은 미확인: CSV 실제 다운로드 후 정확한 행수/용량, 컬럼명이 API JSON 필드와
  일치하는지, 카테고리 필터링/오분류 휴리스틱을 서버로 이관할지 여부.
