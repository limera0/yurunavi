...
### 8. 브랜드 방향성 확정 — DONE (2026-07-29)
- 컬러 팔레트 / 타이포그래피(폰트) / 아이콘 스타일 / 무드. Claude가 방향 2~3개 제안,
  최종 선택은 사용자.
- **2026-07-14 진행**: Google Stitch(stitch.withgoogle.com) 자가 탐색 경로 시도.
  준비 자료 `loop/STITCH_DESIGN_PROMPTS.md`(마스터 프롬프트 + 9개 테마) /
  `loop/stitch_screens/`(M32F 캡처 5장) — 사용자가 별도 브랜드 아이덴티티 질문지에
  직접 답변하는 쪽으로 대체, Stitch 경로는 보류.
- **2026-07-29 확정**: 사용자가 질문지(`260729_brandIdentity_Yurunavi.md`, 원본은
  인코딩 손상되어 채팅에 재입력받음)에 답변 → Claude가 방향 3안(A/B/C) 제안 →
  사용자가 A안을 기본, B/C도 폐기하지 않고 **3안 전부를 설정 화면에서 고를 수 있는
  무료 스킨**으로 채택. 향후 개성을 더 살린 스킨은 유료 제공 예정(스캐폴딩은
  11번 Phase 5와 연계).
  - **A — 유루캠 무드** (기본/default): 코랄 `#E2896F` + 모스그린 `#8CA283`,
    시마 린(유루캠)·논논비요리·보노보노 계열 무드.
  - **B — 레트로 모터링**: 민트 `#78B4AC` + 카키골드 `#C7A768` + 더스티로즈 `#DFA79E`,
    베스파 프리마베라 × 닛산 피가로/파오 무드.
  - **C — 동네 라이딩 메이트**: 브릭레드 `#C05F4C` + 포레스트그린 `#4F6B58` +
...
조회하는 방식 채택. 공식 API 키 발급은 더 이상 선행 조건이 아님 — `.env`의 `OPINET_API_KEY`는
발급은 됐지만 이 구현 경로에서는 코드 어디서도 참조되지 않음(2026-07-29 확인).

**구현 (커밋 `d93a553`/`5d1716b`/`99d737c`, 2026-07-25)**:
- 백엔드: `native/src/main.rs` `GET /gasstations/nearby?lat=&lon=&fuel=&radius_m=` —
  역지오코딩 → Opinet 내부 API 조회 → GIS 좌표 역변환 → 반경 필터 → 가격 오름차순 상위 20건
  반환. 단위테스트(`gis_to_wgs84_converts_known_seoul_stations`) 통과, 실측 확인(서울시청
  5km 반경 6건 반환).
- 프론트엔드: `lib/services/gas_station_service.dart`(신규) + 내비 화면(`nav_screen.dart`)
  우측 주유소 아이콘 버튼 → 바텀시트(`_GasStationSheet`, 휘발유/고급휘발유 토글칩, 거리·가격순
  리스트) → 선택 시 "경유지로 설정" 흐름(현위치 기준 haversine 정렬 삽입,
...
### 19번 후속 (2026-07-30, 같은 날 저녁 세션) — 고급휘발유 99999 버그 수정 + 지도 검색창 진입점 추가

**주의**: 위 "원래 계획 대비 달라진 점"에 적힌 "지도 화면에 별도 진입점을 만들지 않기로
확정"은 **이 세션에서 마스터가 번복**했다 — 같은 날 안에 결정이 두 번 바뀐 것이므로 앞
문단은 낮 시점 기록으로만 남겨두고, 아래가 최신 상태다.

**Phase A — 고급휘발유(B034) 가격 99999원 버그 (커밋 `5353305`)**:
- 원인: 오피넷이 해당 주유소가 고급휘발유를 취급하지 않을 때 `B034_P` 필드에 리터럴
  `"99999"`를 "해당없음" 센티널로 내려주는데, `parse_opinet_price()`(`native/src/main.rs`)가
  `p > 0`만 걸러 이걸 실제 가격으로 통과시키고 있었음. 지방 소형 주유소가 많은 지역(울릉군,
  정선군, 영양군 등 실측 확인)일수록 광범위하게 나타남.
- 수정: `p != 99999` 필터 추가. 여기서 한 걸음 더 나아가 **마스터 지시로 스코프 확장** —
  B034 요청 시 고급휘발유 미취급 주유소는 `premium_price: null`로 두는 게 아니라 **목록에서
  완전히 제외**(`should_include_for_fuel()` 신설 헬퍼, B027 요청은 영향 없음). 휘발유 탭과
  고급휘발유 탭이 완전히 동일한 목록일 필요는 없다는 게 이 결정의 취지.
  - 단위테스트 6종 추가, `cargo test` 148 passed. code-auditor PASS.
  - navi 컨테이너 재빌드·재기동 후 실측 재검증: 울릉군 좌표 B034 조회 → `[]`(전부 미취급이라
    전체 제외), B027 조회 → 정상 3건. 서울 중구(혼재 지역) B034 조회 → premium_price 전부
    정상 범위(2369~3240), 99999 없음.

**Phase B — 지도 검색창("주변 탐색" 시트) "주유소" 단독 선택 시 오피넷 경로 전환 (커밋 `0decff2`)**:
- 대상은 핸드오프 문서(`HANDOFF_0730_gasstation_search_chip_and_premium_bug.md`)가 지목한
  `_PlacesSheet`가 아니라 **`_PoiExploreSheetState`**(`main_map_screen.dart`, "주변 탐색"
  시트, 카테고리 필터칩 보유) — 핸드오프의 클래스명/줄번호는 stale이었음, 코딩 전 실제 파일
  확인으로 정정.
- `_isGasStationOnly`(카테고리 칩이 "주유소" 하나만 선택된 경우, 검색어로 확장되는
  `_effectiveTypes`와는 별개) 조건일 때만 `GasStationService.fetchNearby(fuel: 'B027')`
  경로로 전환 — 그 외 조합(칩 여러 개, 다른 칩, 칩 없음)은 기존 소상공인 DB(`/poi/nearby`)
  경로 100% 그대로.
- 유종은 B027(휘발유) 고정 — 토글칩 추가는 범위 밖(후속 과제로 남김).
- 리스트 아이템은 `nav_screen.dart`의 `_GasStationSheet` 스타일(순번+이름+거리·브랜드+가격)을
  재사용. 결과 탭 시 `GasStation`을 합성 `Poi`로 감싸 기존 `onSelectDest` 콜백 경로를 그대로
  타도록 구현 — 새 콜백/신규 선택 흐름을 만들지 않음.
- `flutter analyze` 통과, code-auditor PASS. `flutter build apk --debug` + `adb install`로
  기기(M32, `RZ8RC1N3V9W`) 설치까지 이 세션에서 완료 — 시트에서 "주유소" 칩 단독 선택 후
  목록 렌더링까지의 실기기 시각 확인은 이 세션에서 좌표 스케일링 문제로 중단, **마스터가
  직접 확인하기로 함(미검증 상태로 남음)**.

**남은 것**:
- Phase B 실기기 시각 확인(마스터 예정) — 확인 후 이 섹션에 결과 반영 필요.
- 유종 토글칩(B027/B034)을 지도 검색창 쪽에도 추가할지는 Phase B 완료 후 마스터 판단 대기
  (핸드오프 원문 지시사항, 아직 미착수).
...
**배경**: `docs/privacy_policy.md`(6번 항목)가 원래 "개인위치정보 수집·이용·제공사실
확인자료 6개월 보관"을 명시하고 있었으나, 2026-07-29 외부 법률 검토 반영 세션에서
실제로 이를 기록하는 코드가 `native/src/main.rs`에 전혀 없다는 사실이 드러남(요청
로깅 미들웨어 자체가 없음). "실제로 안 하는 걸 문서에 주장하지 말라"는 마스터 지시에
따라, 문서 쪽은 해당 문구를 "확인자료를 보관하지 않는다"로 하향 정정 완료
(`docs/privacy_policy.md` + `native/src/main.rs`의 `PRIVACY_HTML` 양쪽 반영).

**남은 문제**: 「위치정보의 보호 및 이용 등에 관한 법률」 제16조는 위치기반서비스
사업자에게 개인위치정보 수집·이용·제공사실 확인자료를 위치정보시스템에 **자동
기록·보존(6개월 이상)**할 의무를 부과한다. 이는 이용자가 요청할 때만 대응하면 되는
것이 아니라, 사업자로 정식 등록되면 상시 갖추고 있어야 하는 기술적 의무다.

**할 일**: 6번 항목의 "위치기반서비스사업 신고"가 실제로 완료(또는 착수)되는 시점에
맞춰, navi 백엔드(`native/src/main.rs`)에 최소한의 요청 로그 테이블을 추가할 것 —
요청 시각·개략적 요청 유형 정도만 기록하고 개인식별정보는 포함하지 않는 최소 설계로
충분하다. 21번(운영 서버 보안 강화)에서 지적된 "접근 로그가 전혀 없다"는 문제와
근본 원인이 같으므로, 가능하면 21번의 요청 로깅 작업(`tower_http::TraceLayer` 등)과
묶어서 함께 처리하는 것을 권장.
...
읽기 전용 조사만 진행했고 리스크 있는 변경(방화벽 활성화 등)은 실행하지 않음.
상세 근거는 `loop/HANDOFF_0729_server_security_audit.md` 참고.

**확인된 사실 (우선순위순)**:
1. `ufw` 방화벽이 꺼져 있고(`ENABLED=no`), navi(8003)·valhalla(8002)·tiles(8080)·
   style-ai(8014) 서비스가 `docker-compose.yml`에서 전부 `0.0.0.0`에 직접 바인딩되어
   있음. Cloudflare Tunnel 경유 도메인 접속과 별개로, 클라우드 공급자 단에 별도
   보안그룹이 없다면 공인 IP:포트로 직접 접근 가능한 상태로 추정됨(서버 자체 curl
   테스트는 hairpin NAT로 결론 못 냄 — 외부망에서 재확인 필요).
2. navi/valhalla/tiles 어느 API도 인증 헤더나 속도 제한이 없음 — 경쟁사가 앱을
   디컴파일할 필요 없이 트래픽 관찰만으로 fun-road 스코어링 로직을 블랙박스 프로빙
   가능, 동시에 봇 스캐닝/DoS에도 취약.
3. `native/.env`(VWorld API 키 포함)가 644(다른 로컬 계정도 읽기 가능)로 방치돼
   있었음 — 2026-07-29 세션 중 600으로 즉시 수정 완료(무위험 로컬 조치).
4. 요청/접근 로그가 서버에 전혀 없음 — 20번 항목과 근본 원인 동일.
5. 양호한 부분(유지만 하면 됨): SSH는 키 인증만 쓰는 것으로 추정(cloud-init 설정
   파일 크기로 정황상 확인, 100% 확정은 아님), OS 자동 보안패치(`unattended-upgrades`)
   활성화됨, SQL 인젝션 경로 없음(rusqlite 파라미터 바인딩 확인), ProGuard/R8 이미
   적용됨(4번 항목).

**권장 우선순위 (실행은 전부 마스터 확인 후, 단계별로)**:
1. Cloudflare 대시보드에서 WAF/속도제한/Bot Fight Mode 켜기 — 서버 코드 무변경,
   리스크 최소, 즉시 가능. (계정이 별도 프로젝트인 `/data/n8n-stack/` 소유라 이
   리포에서 직접 조작 불가 — 마스터가 대시보드에서 진행.)
2. API에 최소 공유 키(헤더) 인증 추가 — Flutter 앱 + Rust 서버 동시 배포 필요.
3. `ufw` 활성화 + 8002/8003/8080/8014를 도커 브리지 서브넷(`docker_default`=
   172.19.0.0/16 확인함)과 실제 필요한 경로에서만 허용 — **SSH 락아웃/사이트 다운
   위험 있어 콘솔 접속 확보한 채로 단계적 진행 필수**, 절대 원격 세션만으로 일괄
   적용하지 말 것.
4. `tower_http::TraceLayer` 등으로 요청 로깅 추가(20번과 함께 처리 권장).
5. fail2ban 설치(SSH 방어 심화 — 이미 키인증이라 우선순위 낮음).

**전제**: "완벽 차단"은 어떤 시스템에도 불가능 — 목표는 계층적 방어로 리스크를 실질적
수준까지 낮추는 것.

**진행 상황 (2026-07-30)**: 마스터가 2순위(API 공유키 인증)부터 진행하기로 선택,
완료함. 상세는 `loop/HANDOFF_0730_navi_api_key_auth.md` 참고. navi(8003) API는
이제 `/health`·`/privacy`를 제외한 전 엔드포인트가 `X-Api-Key` 헤더(상수시간 비교,
`NAVI_API_KEY` 환경변수)를 요구한다 — 커밋 `bc1fa0c`(Rust 미들웨어) ·
`c72f873`(Flutter 클라이언트 헤더 부착, `--dart-define-from-file=env.json`).
운영 navi 컨테이너 재빌드/재기동 완료, curl로 401/200 동작 확인, 디버그 APK
재빌드 후 테스트 기기(RZ8RC1N3V9W)에 설치해 지도 화면에서 POI(편의점/마트)가
정상 표시됨을 스크린샷으로 확인함 — 앱이 실제로 새 헤더로 인증에 성공하고 있다는
뜻. valhalla(8002)·tiles(8080)는 이번 작업 범위 밖(서드파티 이미지라 앱단 헤더로
못 막음, 1순위 Cloudflare 또는 3순위 ufw로 다뤄야 함).

**진행 상황 (2026-07-30, 같은 날 저녁 — 1순위)**: 마스터가 Cloudflare 대시보드에서
직접 진행(계정이 `/data/n8n-stack/` 소유라 Claude는 접근 불가, 전 과정 마스터 실행).
무료(Free) 플랜 제약 안에서 다음 두 가지로 확정:
- **Rate limiting rule**(무료 쿼터 1개, 전량 사용): `navi.westinx.com` +
  `valhalla.westinx.com` 대상, IP당 30요청/10초 초과 시 10초간 Block. tiles는
  버스트 트래픽 패턴이 달라(지도 팬/줌 시 타일 수십 개 동시 요청) 같은 규칙에
  묶지 않고 이번엔 제외 — 원 감사 우려("경쟁사가 트래픽 관찰로 fun-road 스코어링
  로직 프로빙 가능")가 navi/valhalla에만 해당하고 tiles는 공개 OSM 이미지라 보호
  가치가 낮다는 판단.
- **Custom rule**(무료 쿼터 5개 중 1개 사용): navi/valhalla/tiles 세 도메인 모두
  대상, `ip.geoip.country`가 KR·JP가 아니면 Block(한 규칙으로 화이트리스트 처리).
  국내 전용 오토바이 투어링 앱이라는 전제, 해외 로밍 사용 계획 없음 확인 후 적용.
- **WAF Managed Rules(OWASP Core Ruleset)**: Pro 플랜($25/월) 전용 확인, 무료
  유지 방침이라 스킵.
- **Bot Fight Mode**: 켜지 않기로 결정 — JS Challenge 방식이라 브라우저가 아닌
  앱의 직접 HTTP 호출은 못 풀어서, 봇이 아니라 정상 앱 트래픽을 막을 위험만 있고
  얻는 이득은 없음(세 도메인 다 브라우저 대상 웹페이지가 아니라 API/타일 서빙 전용).
