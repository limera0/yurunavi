...
| # | 하위 과제 | 근거 요약 | 상태 |
|---|-----------|-----------|------|
| 13-1 | POI 탐색 UI | 백엔드 완성·미사용 상태, UI만 필요 → 최저비용 최고가치 | **DONE** (`2d91e4d`) — data.go.kr API로 데이터소스 교체 완료 |
...
| 13-5 | 백그라운드/오버레이 내비게이션 | 네이티브 포그라운드 서비스+알림, 리스크 최고 | TODO |
...
- `flutter analyze` 0 issues, `flutter test` 96/96, `flutter build apk --debug
  --dart-define-from-file=env.json` 빌드 성공. **⚠️ 헤드리스 서버라 실기기 육안 확인은 여전히
  못함** — 다음에 노트북에서 `adb install`로 검색바 UX/카테고리 칩/POI 핀 렌더링 실사용 확인
  권장.
