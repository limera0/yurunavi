...
> **완료됨(2026-07-30 밤) — 이 문서는 참고용으로만 남긴다.** 실제로는 마스터가
> "SSH 말고 콘솔이 없고 명령어 직접 치는 것도 부담스럽다"고 해서, 아래 순서를
> 마스터가 직접 치는 대신 narrow-scope sudoers 위임(`/etc/sudoers.d/
> claude-ufw-fail2ban`)을 한 번 받아 Claude가 데드맨 스위치(자동복구 타이머)와
> 함께 직접 실행·검증까지 완료했다. 결과·최종 규칙은 `RELEASE_ROADMAP.md` 21번
> 섹션 참고. 아래 내용은 "왜 위험한지"와 각 단계 커맨드 자체는 여전히 유효하니
> 다음에 이 서버에서 비슷한 방화벽 작업을 할 때 참고재로 남겨둔다.
