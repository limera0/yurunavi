...
### 상태: **구현 완료 (2026-07-31, 배치4/라운드13→14→15→16 세션, 커밋 `1ffb211`)**
flutter-coder → code-auditor PASS(1회) → 커밋. AppBar 로컬 오버라이드 제거(전역
모스그린 상속), 프로필/언어/스킨 `ListTile` 간격 축소, 프로필 섹션헤더 추가,
지도 표기언어(드롭다운)+지도 방향(세그먼트버튼)+즐겨찾기 카테고리를 "앱 설정"
섹션으로 통합, 스킨 표시명 노을빛/박하빛/황토빛 교체, 안드로이드 앱 이름
ko/ja 로케일 분기(`strings.xml` 3종 + manifest). 코더가 "주행 설정" 섹션에
계획 스냅샷 이후 추가된 "야간 지도 어둡게" 토글이 남아있는 걸 발견해 계획의
"섹션이 비면 헤더 삭제" 조건에 따라 헤더를 유지한 판단은 감사에서 정확함을
확인.
...
### 상태: **구현 완료 (2026-07-31, 배치4/라운드13→14→15→16 세션, 커밋 `a88be5b`)**
flutter-coder → code-auditor PASS(1회) → 커밋. 최근 경로 행에 `_FavoriteStarButton`
재사용 추가, 즐겨찾기/카테고리 삭제에 확인 다이얼로그 추가(카테고리는 미분류
이동 안내 포함), `PlacesService.reassignFavoriteCategory`로 카테고리 삭제 시
소속 즐겨찾기 자동 미분류 재배정(저장 순서 보존 확인됨 — 가장 버그 나기 쉬운
지점이었으나 감사에서 정확함 확인), 카테고리 관리 AppBar 로컬 오버라이드
제거, 카테고리 추가를 인라인 입력으로 전환, `ReorderableListView`+좌측
드래그 핸들로 순서 변경 지원(미분류는 핸들 없이 최상단 고정). 감사에서
기존 히스토리 삭제 다이얼로그(브랜드 오렌지)와 이번 신규 다이얼로그
(`AppColors.error` 빨강)의 색상 불일치를 백로그로 지적함 — 추후 통일 검토
필요.
...
### 상태: **구현 완료 (2026-07-31, 배치4/라운드13→14→15→16 세션, 커밋 `4a9be68`)**
flutter-coder → code-auditor PASS(1회) → 커밋. AppBar 로컬 오버라이드 제거,
`image_picker`+`path_provider`로 로컬 갤러리 아바타 등록(구글 로그인 프로필
사진과 완전 독립 확인됨), 구글 로그인 버튼을 공식 4색 G 로고 SVG(신규 제작,
`assets/images/google_g_logo.svg`)로 재제작, 인스타그램 아이콘을 브랜드
SVG(`assets/icon/insta.svg`를 `assets/images/instagram_icon.svg`로 복사)로
교체해 이중 `@` 해소, 바이크 삭제에 확인 다이얼로그 추가. `image_picker`
갤러리 접근은 최신 버전 기준 안드로이드 권한 불필요 확인(감사에서 검증).
...
### 상태: **구현 완료 (2026-07-31, 배치4/라운드13→14→15→16 세션, 커밋 `64c0ed0`)**
flutter-coder → code-auditor **FAIL 1회(오버플로 위험+`FixedExtentScrollController`
누수) → 수정 → PASS** → 커밋. 브랜드/모델/배기량 placeholder 한글화, 연식
필드를 탭-투-오픈 모달 `CupertinoPicker`(1970~2027)로 전환, `_BikeEditDialog`
→`_BikeEditSheet`로 팝업→바텀업 카드 전환(`_showWaypointSheet` 패턴 재사용).
1차 감사에서 `isScrollControlled` 바텀시트에 스크롤 컨테이너 없이 키보드
패딩만 추가해 작은 화면/키보드 뜬 상태 오버플로 위험을 지적받아
`Flexible`+`SingleChildScrollView`로 수정, 휠피커 컨트롤러 dispose 위치도
수정 후 재감사 PASS(dispose 타이밍 안전성은 위젯 테스트로 실증 검증됨).
