...
## 먼저 읽을 것 (순서대로)
1. AGENTS.md (항상 적용되는 작은 공유 kernel)
2. CLAUDE.md (Claude 어댑터와 선택적 `.ai/` 로딩 경로)
3. 오늘 밤 작업 지시서: ${TASK_FILE}
4. 직전 틱이 남긴 진행 메모: ${HANDOFF_FILE} (없으면 오늘 밤 첫 틱 — 3번부터 시작)
5. 진짜 git 상태를 직접 확인해라: git log --oneline -10, git status.
   진행 메모의 "이미 했음" 서술을 그대로 믿지 마라 — 커밋 해시/타임스탬프로
   직접 검증해라 (git show -s --format='%ci %h %s' <hash>).
   작업 디렉토리가 지저분하면 소유권을 handoff와 Git으로 확인해라. 이번 작업 소유임이 입증된 변경만 이어서 완료하고, 소유권이 불명확한 변경은 절대 수정·되돌림·스테이징하지 마라.

## 이번 틱에서 할 일
- 오늘 밤 작업 전체를 끝내려 하지 마라. 딱 한 개의 체크포인트 단위만 진행해라
  (대략: 코더 위임 1회 + 감사 1회 + 커밋 1회, 막혔으면 조사 후 보고로 대체).
- AGENTS.md를 kernel로 따르고 이번 체크포인트에 필요한 `.ai/` 문서만 선택해 읽는다. 구현은 `.ai/WORKFLOW.md`, 커밋은 `.ai/GIT_SAFETY.md`, 보고는 `.ai/REPORTING.md`를 따른다.
- 기술적으로 애매하면 코드·테스트·Git·기존 기록을 먼저 조사하고 안전한 범위에서 스스로 결정해 계속한다. AGENTS.md의 에스컬레이션 조건에 해당할 때만 사람에게 묻는다.
...
쓰지 말고, 확인한 사실·제품 영향·AI 권장안을 먼저 적고 Product Owner만 답할 수 있는 질문 하나를 쉬운 한국어로 적어라. 기술 선택을 떠넘기지 마라.
...
- `.ai/REPORTING.md`를 읽고, 맨 위 `## 1분 요약`은 바쁜 비개발자 Product Owner가 읽을 쉬운 한국어로 쓴다. 기술 기록은 `## AI 인수인계` 아래로 분리한다.
- **[필수] 리포트에 반드시 아래 한 줄(달성도 판정, `.ai/REPORTING.md`)을 포함해라:**
...
- `.ai/REPORTING.md` 형식의 `## 1분 요약`
- `## AI 인수인계`에 커밋, 정확한 검증, 중단 이유, 이어할 단계
- **[필수] 아래 한 줄(달성도 판정, `.ai/REPORTING.md`)을 포함해라:**
  \`**목표 달성 판정:** 원래 목표: <오늘 목표 한 줄> / 달성: 예·부분·아니오 — <근거 한 줄>\`
  목표는 작업지시서(${TASK_FILE})나 handoff의 "🎯 오늘 목표 확정" 섹션에서 가져와 정직하게 판정.
- 바쁜 비개발자 Product Owner가 1분 안에 이해할 수 있는 쉬운 한국어로 써라.
PROMPT
}
