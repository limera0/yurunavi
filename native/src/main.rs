...
<ul>
  <li>경로 탐색·지도 표시를 위해 GPS 좌표가 회사가 직접 운영하는 지도 타일·경로 탐색 서버(westinx.com 하위 도메인)로 전송됩니다. 이 서버는 외부 제3자가 아닌 회사가 직접 관리하며, 수신한 위치 정보는 경로 계산에 사용된 즉시 폐기하고 별도로 저장·분석·활용하지 않습니다. 회사는 개인위치정보 수집·이용·제공사실에 관한 별도의 확인자료를 보관하지 않으며, 그 외 서비스 운영·보안을 위한 최소한의 서버 접속기록(개인위치정보를 식별 가능한 형태로 포함하지 않는 오류 로그 등)이 관련 법령에 따라 일정기간 보관될 수 있습니다.</li>
</ul>
...
<ul>
  <li><strong>위치정보(경로 계산용 GPS 좌표)</strong>: 실시간 경로 계산에만 이용되며, 계산 완료 후 별도 데이터베이스에 저장하지 않습니다. 개인위치정보 수집·이용·제공사실에 관한 별도의 확인자료도 보관하지 않습니다.</li>
...
<ul>
  ...
  <li><strong>이전 거부 방법 및 효과</strong>: 크래시 리포팅은 서비스 안정성 확보를 위한 필수 기능으로 운영되며 별도의 개별 거부 기능은 제공하지 않습니다. 이용자는 앱 이용을 중단(삭제)함으로써 해당 개인정보 처리에 대한 동의를 언제든지 철회할 수 있습니다. 그 밖에 문의사항은 11번 연락처로 연락해 주시기 바랍니다.</li>
</ul>
...
<ul>
  ...
  <li>개인정보의 열람·정정·삭제 요청: 아래 11번 이메일로 요청할 수 있으며, 회사는 「개인정보 보호법」 제35조~제38조에 따라 법령에서 정한 특별한 사유가 없는 한 지체 없이 필요한 조치를 취합니다.</li>
...
<ul>
  ...
  <li>회사는 위치정보 자체(GPS 좌표값) 및 개인위치정보 수집·이용·제공사실에 관한 별도의 확인자료를 보관하지 않으므로 이에 대한 파기 대상 데이터가 발생하지 않습니다.</li>
  <li>다만 서비스 운영·보안을 위해 보관하는 최소한의 서버 접속기록(오류 로그 등)이 있는 경우, 전자적 파일 형태로 보관된 정보는 기록을 재생할 수 없는 기술적 방법을 사용하여 삭제합니다.</li>
  <li>본 서비스는 개인정보를 출력물(서면) 형태로 보관하지 않습니다.</li>
</ul>
...
<ul>
  <li><strong>기술적 조치</strong>: 단말기-서버 간 통신 구간 암호화(HTTPS), 서버 관리자 계정에 대한 비밀번호 방식이 아닌 키 기반 인증 적용, 자체 운영 서버에 대한 접근 권한 제한</li>
...
<ul>
  <li><strong>8세 이하의 아동 등의 보호의무자 권리</strong>: 8세 이하의 아동, 피성년후견인 등 개인위치정보주체를 사실상 보호할 법률상 의무가 있는 자(보호의무자)가 이들의 생명·신체보호를 위해 개인위치정보의 이용 또는 제공에 동의하는 경우, 본인의 동의가 있는 것으로 봅니다. 보호의무자는 서면 동의서에 의해 동의하며, 언제든지 동의를 철회할 수 있습니다. 다만 회사는 개인위치정보 수집·이용·제공사실에 관한 별도의 확인자료를 보관하지 않으므로 관련 열람·고지 요청에 응할 자료가 없으며, 현재 보호의무자 동의를 접수·관리하는 별도 절차도 갖추고 있지 않으므로, 위 대상자는 보호자와 함께 이용하시기 바랍니다.</li>
