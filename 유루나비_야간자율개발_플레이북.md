...
3. 아래 명령으로 무인 세션 시작:
   ...
   ```
   # 일반 사용자 계정에서 (root 아님!), 프로젝트 폴더 안에서
   claude --permission-mode auto
   
   # 그리고 첫 프롬프트로:
   "CLAUDE.md와 NIGHT_TASK.md를 읽고, 오케스트레이터로서
    오늘 밤 작업만 수행해. 코딩은 flutter-coder/rust-coder에게,
    검사는 code-auditor에게 위임해. 끝나면 MORNING_REPORT.md를 써.
    범위 밖 작업은 하지 마. 막히면 멈추고 보고서에 적어."
   ```

> `auto` 모드가 가끔 옵트인 프롬프트를 띄울 수 있으니, 처음 한 번은 켜고 수락한 뒤
> 자리를 비우세요. 완전 무인 직전에 한 번 테스트 실행으로 확인하는 걸 권합니다.

---

## 7. 일어나서 검토 (오후 1시, 5분)

1. `MORNING_REPORT.md` 읽기 — 무엇이 됐고, 뭐가 막혔는지.
